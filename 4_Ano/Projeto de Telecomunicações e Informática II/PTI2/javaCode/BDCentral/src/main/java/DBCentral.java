...
//            connec = DriverManager.getConnection(SQLPath,SQLuser,SQLpass);
    ...
    public void populateuser() throws ParseException {
        ...
        try {
            ...
            String nome2 = "carro";
            int typeUser2 = 2;
            String carro = "INSERT INTO utilizador(nome,typeUser,username,password,email,address,birthday,date_registration) values('" + nome2 + "','" + typeUser2 + "','" + username + "','" + generatedSecuredPasswordHash + "','" + email + "','" + address + "','" + dateAlert + "','" + date_registration_S + "')";
            ps = connec.prepareStatement(carro);
            ps.executeUpdate(carro);
    ...
    public String getUser(String id){
        ...
        try {
            ...
            while (rs.next()) {
                ...
                res = "id_user: " + idUser + "," + "nome: " + nm + "," + "typeUser: " + tipo + ","  + "username: "  + user + "," + "email: " + mail + "," + "address: " + addr + "," + "birthday: " + birth + "," + "date_registration: " + reg;
    ...
    public boolean login (String user, String pass) {
        ...
        try {
            ...
            String query = "SELECT * FROM utilizador WHERE username = '" + usern + " '";
            rs = st.executeQuery(query);
            res=rs.next();

            String confirmpass = rs.getString("password");
            System.out.println(confirmpass);
            boolean matched = SCryptUtil.check(pass, confirmpass);
    ...
    public int getID_alerta(String TimeStamp){
        String resposta = null;
        int id_alert = 0;
        try {
            st = connec.createStatement();
            String query = "SELECT * from alert WHERE dateAlert = '" + TimeStamp + " '";
            rs = st.executeQuery(query);
            while (rs.next()) {
                Date dateAlert = rs.getDate("dateAlert");
                int id_localCity = rs.getInt("id_localCity");
